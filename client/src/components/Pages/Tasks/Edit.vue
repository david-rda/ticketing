    ...
    <div>
        ...
        <div class="container mt-5 custom-container bg-white">
            <form method="POST" @submit.prevent="edit_task()">
                ...
                <div class="mb-3">
                    <file-pond allow-multiple="true" :server="'http://localhost:8000/api/task/file/upload/' + this.$route.params.id" />
                </div>
                <div class="mb-3">
...
<script>
    ...
    export default {
        ...
        async mounted() {
            ...
            const loaded_task = await axios.get("http://localhost:8000/api/task/get/" + this.$route.params.id, {
                headers : {
                    "Authorization" : `Bearer ${this.$store.state.token}`
                }
            });
        ...
        methods : {
            async edit_task() {
                try {
                    const create_task = await axios.put("http://localhost:8000/api/task/edit/" + this.$route.params.id, {
                        title : this.title,
